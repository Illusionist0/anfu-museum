...
<template>
  ...
  <div class="arrow_left" @click="handlePrevious" v-if="num == 0 || num == 1">
    <svg-icon class="navigate_svg" icon-class="left" />
  </div>
  <div class="arrow_right" @click="handleNext" v-if="num == 1 || num == 2">
    <svg-icon class="navigate_svg" icon-class="right" />
  </div>
...
<script>
import {get3dInfo, getInfoapi, listAfxCollectionByYearsAnd} from "@/api/afxmanage/material"
import {AmbientLight, Color, PerspectiveCamera, Scene,MeshBasicMaterial, TextureLoader,Mesh,ImageUtils,cancelAnimationFrame,Vector3, WebGLRenderer} from "three";
...
export default {
  ...
  mounted() {
    this.init()
    this.$nextTick(() => {
      this.createModel('7b6ab617-5ec9-4cef-a1e2-55582e501a10')
    })
  ...
  methods: {
    ...
    get3dInfo() {
        get3dInfo().then(res => {
          ...
          console.log('看看',this.infoList[this.num].name);
    ...
    createModel(data){
      ...
      getInfoapi({id:id}).then(async response => {
        ...
        tht.file3dMp3 = tht.baseURL + response.data.file3dMp3;
    ...
    init(){
      ...
      // this.renderer.setClearColor('#8e8a8bd4', 0.4);
      //实例化相机
      ...
      handlePrevious() {
        console.log("pre >>>> ");
        this.num++
        this.refresh(this.list[this.num])
      },
      handleNext() {
        console.log("next >>>> ");
        // this.clearModel()
      ...
      clearModel () {
        // if (this.scene !== null && this.scene.children.length > 3) {
          ...
          const domDiv = document.getElementById('three-canvas')
          if (domDiv !== null && domDiv.firstChild) {
...
#three-canvas {
    ...
    height: 100vh;
    overflow: hidden;
    /* background: linear-gradient(rgb(0, 0, 1), rgb(119, 119, 237)); */
}
.collection-text{
  position: absolute;
  top: 30px;
